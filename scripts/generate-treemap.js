...
const ignoreDirs = ["node_modules", "dist", ".git", "dev-dist", "Recursos"];
const ignoreExts = [".log", ".md", ".lock", ".png", ".webp", ".jpg", ".jpeg", ".gif", ".svg", ".ico"]; // extensiones a ignorar
...
function printTree(dir, prefix = "", depth = 2) {
  ...
  items.forEach((item, index) => {
    ...
    if (statSync(fullPath).isFile() && ignoreExts.includes(path.extname(item))) {
      return;
    }
