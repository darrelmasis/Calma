  ...
  const handleSubmit = async () => {
    ...
    try {
      if (isOffline) {
        ...
        toast.info('📨 Sin conexión. Tu reserva se guardó en la bandeja de salida y se enviará automáticamente cuando vuelva internet.')
      ...
      console.error(err)
      await addToQueue(payload, 'booking')
      toast.warning('⚠️ No se pudo enviar la reserva. Se guardó en la bandeja de salida y se enviará automáticamente cuando haya conexión.')
  ...
  return (
    <section className='bg-white py-5'>
      <div className='container d-flex justify-content-center booking-section'>
        <Stepper formData={formData} setFormData={setFormData} onSubmit={handleSubmit} isSubmitting={isLoading} isCompleted={isSuccess}>
          <StepsContent>
                ...
                <div className='d-flex flex-direction-column align-items-center justify-content-center'>
                  ...
                  <div className='d-flex w-100 flex-wrap-wrap gap-2 justify-content-center'>
                    {Object.entries(servicesWithInfo).map(([cat, services]) => {
                      ...
                      return (
                        <div key={cat} className='rounded border p-3 w-100 max-wx-md-300'>
                  ...
                  <div className='max-wx-lg-700 p-3 mt-5 text-center d-flex flex-direction-column gap-1'>
                    ...
                    <div className='text-muted fw-light'>
                      {`${totalServices} ${
                        totalServices === 1 ? t('header.dropdown.totalSubtitle') : t('header.dropdown.totalSubtitle') + 's'
                      }`}
                    </div>
                  </div>
                </div>
            ...
            <Step stepKey='personalData' className='d-flex flex-1 justify-content-center mt-3'>
              {({ formData, setFormData, getField, validateField, clearField }) => (
                <div className='d-flex flex-direction-column justify-content-center gap-2 flex-1 max-wx-500'>
                  ...
                  <PhoneNumber
                    ...
                    onChange={(valObj) => {
                      ...
                      setTimeout(() => validateField('phone', valObj?.formatted || '', formData), 500)
                    }}
                    onBlur={() => validateField('phone', formData.phone, formData)}
