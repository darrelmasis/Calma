  ...
  const totalPrice = useMemo(() => {
    return Object.entries(services).reduce((acc, [categoryId, items]) => {
      const sum = items.reduce(
        (s, { subCategoryId, serviceId }) => s + getServicePrice(subCategoryId, serviceId),
        0
      );
      return acc + sum;
    }, 0);
  ...
  const servicesWithInfo = useMemo(() => {
    ...
    Object.entries(services).forEach(([categoryId, items]) => {
      ...
      result[categoryId] = items.map(({ subCategoryId, serviceId }) => {
        ...
        const serviceDescription = t(`services.section_1.category.${categoryId}.subCategory.${subCategoryId}.services.${serviceId}.description`);
        const servicePrice = getServicePrice(subCategoryId, serviceId);
        return { id: serviceId, categoryName, subCategoryName, serviceName, serviceDescription, servicePrice, subCategoryId };
      });
    });
    return result;
  }, [services, t]);
